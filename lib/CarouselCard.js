...
export const CarouselCard = ({ init, item, active, width, interval, onPress, }) => {
    ...
    return (<TouchableWithoutFeedback onPress={onPressItem}>
      <View style={styles.shape}>
        <View style={styles.imageWrapper}>
          {!!item.imageUrl && (<Image key={item?.landingUrl} style={styles.image} source={{ uri: item.imageUrl }}/>)}
...
export const getStyles = (width, interval, backgroundColor) => StyleSheet.create({
    image: {
        height: '100%',
        width: '100%',
    },
